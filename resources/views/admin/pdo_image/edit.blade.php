...
<div class="Hui-article-box">
	<form class="form form-horizontal" id="form-article-add" method="post" action="{{asset('/admin/pdo_image/update')}}" enctype="multipart/form-data">
		<input type="hidden" value="{{$list['id']}}" name="id">
		<input type="hidden" value="{{$list['pdo_id']}}" name="pdo_id">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>图片名字：</label>
			<div class="formControls col-xs-8 col-sm-5">
				<input type="text" class="input-text" value="{{$list['title']}}" placeholder="" id="" name="title">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-5">
				<select name="status" id="">
						<option value="1"@if($list->getOriginal('status')==1)
						selected
						@else
						''
						@endif
                        ...
                        <option value="2"@if($list->getOriginal('status')==2)
						selected
						@else
						''
						@endif
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				...
				<button onClick="layer_close();" class="btn btn-default radius" type="reset"><a href="{{asset('admin/pdo_image/index')}}">&nbsp;&nbsp;取消&nbsp;&nbsp;</a></button>
			</div>
		</div>
		{{csrf_field()}}
	</form>
