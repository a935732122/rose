			...
			<div class="mu_content_wrap">
   				<div class="order-title">
        			<ul class="order-title-column clearfix">
			            <li style="width:25%"><a href="{{asset('home/personal/index')}}" style="color:black">全部订单</a></li>
			            <li style="width:25%"><a href="{{asset('home/personal/dfx')}}" style="color:red">待付款</a></li>
			...
			@if($imx['id'])
	    		...
	    		<div>
	    			...
	    			@if($v['did']==$sx['id'])
	    			 @foreach($p as $s)
			            @if($v['pid']==$s['id'])
			            <div style="width:15%;padding-top: 50px;">{{$v['updated_at']}}</div> 
			            	@endif
		            	@endforeach
			            	@endif
			        @endforeach
	    			...
	    			@if($v['did']==$sx['id'])
			            @foreach($p as $s)
			            @if($v['pid']==$s['id'])
		            	<div style="width:150px;padding-left: 100px;padding-top: 50px">{{$s['name']}}</div>
		             	@endif
		            	@endforeach
			            	@endif
			        @endforeach
	    			...
	    			@if($v['did']==$sx['id'])
			            @foreach($p as $s)
			            @if($v['pid']==$s['id'])
			            <div style="width:15%;padding-top: 50px;padding-left: 50px">{{$s['price']}}</div>
			         	@endif
		            	@endforeach
			            	@endif
			        @endforeach
		           ...
		           <div style = "float: left;">
					...
					@foreach($im as $v)
					@if($v['did']==$sx['id'])
			            <div style="width:15%;padding-top: 50px;padding-left: 80px">{{$v['pnum']}}</div>
			         @endif
			        @endforeach
			        @endforeach
		           </div>
					...
					@foreach($im as $v)
					@if($v['did']==$sx['id'])	
			     			<div style="width:150px;padding-top: 50px;padding-left: 100px">未付款<a href="{{asset('home/shop/much')}}/{{$sx['id']}}" style="color:red;padding-left: 20px">去付款</a></span></div>
			     	 @endif
			        @endforeach
			        @endforeach		
		           	</div>
